...
class Vale(models.Model):
    ...
    name = fields.Char(string="Secuencia interna", required=True, copy=False, readonly=True, default=lambda self: _("Nuevo"))
    date = fields.Date(string="Fecha", required=True, default=fields.Date.context_today, readonly=True)
    user_id = fields.Many2one("res.users", string="Usuario creación", required=True, default=lambda self: self.env.user, readonly=True)
    ...
    invoice_count = fields.Integer(string="Cantidad de facturas", compute="_compute_invoice_count")
    ...
    def write(self, vals):
        protected_fields = {"date", "partner_id", "amount", "note", "company_id", "currency_id", "user_id"}
        for record in self:
    ...
    def action_create_invoice(self):
        ...
        product = self.env["product.product"].search([("default_code", "=", "VALE")], limit=1)
        if not product:
